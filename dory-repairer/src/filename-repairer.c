...
{
    dory_filename_repairer_register_type(module);
    type_list[0] = DORY_TYPE_RENAME_AS;

    bindtextdomain(GETTEXT_PACKAGE, GNOMELOCALEDIR);
    bind_textdomain_codeset(GETTEXT_PACKAGE, "UTF-8");

    dory_filename_repairer_on_module_init();
}

void
dory_module_shutdown(void)
{
    dory_filename_repairer_on_module_shutdown();
}
