...
#include "dory-filename-repairer.h"
...
static void
on_rename_menu_item_activated(DoryMenuItem *item, gpointer *data)
{
    gboolean res;
    gchar* new_name;
    GFile* file;
...
static void
on_repair_dialog_activated(DoryMenuItem* item, gpointer data)
{
    GList* files;
    guint i;
    guint n;
    ...
    argv[0] = g_strdup("dory-filename-repairer");

    i = 1;
    while (files != NULL) {
	GFile* file;

	file = dory_file_info_get_location(files->data);
...
repair_dialog_menu_item_new(GList* files)
{
    ...
    files = dory_file_info_list_copy(files);

    item = dory_menu_item_new(name, label, tooltip, NULL);
    g_object_set_data_full(G_OBJECT(item), "Repairer::files",
	    files, (GDestroyNotify) dory_file_info_list_free);
	...
	GtkWidget* window, gboolean is_submenu)
{
    ...
    item = dory_menu_item_new(id, label, tooltip, NULL);
	     ...
	     const char* name, GFile* file, GtkWidget* window)
{
	...
	if (have_item == NULL) {
	    if (submenu == NULL)
		submenu = dory_menu_new();

	    item = rename_menu_item_new(new_name, file,
			    menu_index, window, TRUE);
	    dory_menu_append_item(submenu, item);
	...
	item = dory_menu_item_new(menu_id,
			      _("Select a filename"),
...
need_repair_dialog(GList* files)
{
    ...
    while (files != NULL) {
	if (dory_file_info_is_directory(files->data))
	    return TRUE;

	name = dory_file_info_get_name(files->data);
...
static GList *
dory_filename_repairer_get_file_items(DoryMenuProvider *provider,
					  GtkWidget            *window,
					  GList                *files)
{
    DoryMenuItem* item;
...
static GList *
dory_filename_repairer_get_name_and_desc (DoryNameAndDescProvider *provider)
{
    GList *ret = NULL;
    gchar *string = g_strdup_printf ("dory-filename-repairer:::%s",
...
static void
dory_filename_repairer_menu_provider_iface_init(DoryMenuProviderIface *iface)
{
    iface->get_file_items = dory_filename_repairer_get_file_items;
}

static void
dory_filename_repairer_nd_provider_iface_init (DoryNameAndDescProviderIface *iface)
{
    iface->get_name_and_desc = dory_filename_repairer_get_name_and_desc;
}

static void 
dory_filename_repairer_instance_init(DoryFilenameRepairer *instance)
{
}

static void
dory_filename_repairer_class_init(DoryFilenameRepairerClass *klass)
{
}

GType
dory_filename_repairer_get_type(void) 
{
    return filename_repairer_type;
}

void
dory_filename_repairer_register_type(GTypeModule *module)
{
    static const GTypeInfo info = {
	sizeof(DoryFilenameRepairerClass),
	(GBaseInitFunc)NULL,
	(GBaseFinalizeFunc)NULL,
	(GClassInitFunc)dory_filename_repairer_class_init,
	NULL, 
	NULL,
	sizeof(DoryFilenameRepairer),
	0,
	(GInstanceInitFunc)dory_filename_repairer_instance_init,
    };

    static const GInterfaceInfo menu_provider_iface_info = {
	(GInterfaceInitFunc)dory_filename_repairer_menu_provider_iface_init,
	NULL,
	NULL
    };

    static const GInterfaceInfo nd_provider_iface_info = {
        (GInterfaceInitFunc) dory_filename_repairer_nd_provider_iface_init,
        NULL,
        NULL
    };
