...
static void
extract_to_callback (DoryMenuItem *item,
		     gpointer          user_data)
{
	GList            *files;
	DoryFileInfo *file;
	...
	default_dir = dory_file_info_get_parent_uri (file);
...
static void
extract_here_callback (DoryMenuItem *item,
		       gpointer          user_data)
{
	GList            *files, *scan;
	GString          *cmd;
...
static void
add_callback (DoryMenuItem *item,
	      gpointer          user_data)
{
	GList            *files, *scan;
	DoryFileInfo *file;
	...
	for (scan = files; scan; scan = scan->next) {
		...
		uri = dory_file_info_get_activation_uri (file);
		quoted_uri = g_shell_quote (uri);
		g_string_append_printf (cmd, " %s", quoted_uri);
		g_free (uri);
		g_free (quoted_uri);
...
static FileMimeInfo
get_file_mime_info (DoryFileInfo *file)
{
	...
	for (i = 0; archive_mime_types[i].mime_type != NULL; i++)
		if (dory_file_info_is_mime_type (file, archive_mime_types[i].mime_type)) {
			char *mime_type;
			char *content_type_mime_file;
			char *content_type_mime_compare;

			mime_type = dory_file_info_get_mime_type (file);
...
static gboolean
unsupported_scheme (DoryFileInfo *file)
{
	gboolean  result = FALSE;
	GFile    *location;
	char     *scheme;

	location = dory_file_info_get_location (file);
...
static GList *
dory_fr_get_file_items (DoryMenuProvider *provider,
			    GtkWidget            *window,
			    GList                *files)
{
	GList    *items = NULL;
	...
	if (unsupported_scheme ((DoryFileInfo *) files->data))
		return NULL;
	...
	for (scan = files; scan; scan = scan->next) {
		...
		if (can_write) {
			...
			parent = dory_file_info_get_parent_info (file);
			can_write = dory_file_info_can_write (parent);
	...
	if (all_archives && can_write) {
		...
		item = dory_menu_item_new ("DoryFr::extract_here",
					       _("Extract Here"),
	...
	if (all_archives &&
		...
		item = dory_menu_item_new ("DoryFr::extract_to",
					       _("Extract To..."),
	...
	if (! one_compressed_archive || one_derived_archive) {
		...
		item = dory_menu_item_new ("DoryFr::add",
					       _("Compress..."),
...
static GList *
dory_fr_get_name_and_desc (DoryNameAndDescProvider *provider)
{
    GList *ret = NULL;
    gchar *string = g_strdup_printf ("dory-fileroller:::%s",
...
static void
dory_fr_menu_provider_iface_init (DoryMenuProviderIface *iface)
{
	iface->get_file_items = dory_fr_get_file_items;
}

static void
dory_fr_nd_provider_iface_init (DoryNameAndDescProviderIface *iface)
{
    iface->get_name_and_desc = dory_fr_get_name_and_desc;
}

static void
dory_fr_instance_init (DoryFr *fr)
{
}

static void
dory_fr_class_init (DoryFrClass *class)
{
...
void
dory_fr_register_type (GTypeModule *module)
{
	static const GTypeInfo info = {
		sizeof (DoryFrClass),
		(GBaseInitFunc) NULL,
		(GBaseFinalizeFunc) NULL,
		(GClassInitFunc) dory_fr_class_init,
		NULL,
		NULL,
		sizeof (DoryFr),
		0,
		(GInstanceInitFunc) dory_fr_instance_init,
	};

	static const GInterfaceInfo menu_provider_iface_info = {
		(GInterfaceInitFunc) dory_fr_menu_provider_iface_init,
		NULL,
		NULL
	};

    static const GInterfaceInfo nd_provider_iface_info = {
        (GInterfaceInitFunc) dory_fr_nd_provider_iface_init,
        NULL,
        NULL
    };
