...
############################################################################
##                                                                        ##
## Dory Terminal - A terminal embedded in Dory                            ##
##                                                                        ##
## Dory Terminal is based heavily on Nautilus Terminal:                   ##
...
##                                                                        ##
## Dory Terminal : http://github.com/linuxmint/dory-extensions/           ##
## Nautilus Terminal: http://projects.flogisoft.com/Dory-terminal/        ##
##                                                                        ##
############################################################################


"""A terminal embedded in Dory."""

__author__ = "Fabien LOISON <flo at flogisoft dot com>"
__maintainer__ = "Will Rouesnel <w.rouesnel@gmail.com>"
__version__ = "1.0"
__appname__ = "dory-terminal"
__app_disp_name__ = "Dory Terminal"
__website__ = "http://github.com/linuxmint/dory-extensions"
...
gettext.bindtextdomain("dory-extensions")
gettext.textdomain("dory-extensions")
_ = gettext.gettext

import gi
gi.require_version('Vte', '2.91')
gi.require_version('Dory', '3.0')
from gi.repository import GObject, Dory, Gtk, Gdk, Vte, GLib, Gio

BASE_KEY = "org.dory.extensions.dory-terminal"
...
class DoryTerminal(object):
    """Dory Terminal itself.
    ...
    def _goto_current_terminal_directory(self):
        """Navigate the active Dory pane to the current working directory
        of the VTE

        This functionality depends on some changes to Dory's extension
        interface.
        """
    ...
    def get_widget(self):
        """Return the top-level widget of Dory Terminal."""
...
#         self.term.set_size(grid_width, grid_height)
    ...
    def set_visible(self, visible):
        """Change the visibility of Dory Terminal.

        Args:
            visible -- True for showing Dory Terminal, False for hiding.
        """
    ...
    def show_about_dialog(self):
        ...
        about_dlg.set_copyright("Dory Terminal is based heavily on Nautilus Terminal by %s Copyright (c) 2011" % __author__)
        about_dlg.set_authors([__author__, __maintainer__])
        logo = Gtk.Image.new_from_file(
            "/usr/share/dory-terminal/logo_120x120.png")
...
class Crowbar(object):
    """Modify the Dory' widget tree when the crowbar is inserted in it.

    Args:
        uri -- The URI of the current directory.
        window -- The Dory' window.
    """
    ...
    def _on_crowbar_parent_set(self, widget, old_parent):
        """Called when the crowbar is inserted in the Dory' widget tree.
    ...
    def _on_crowbar_pp_parent_set(self, widget, old_parent):
        ...
        if not widget.get_parent():
            ...
            if not hasattr(hbox, "nt"):
                print("[%s] W: Can't find the Dory Terminal instance..."
                      % __app_disp_name__)
            hbox.nt.destroy()

class DoryTerminalProvider(GObject.GObject, Dory.LocationWidgetProvider, Dory.NameAndDescProvider):
    """Provides Dory Terminal in Dory."""

    def __init__(self):
        """The constructor."""
        print("[%s] I: Initializing the Dory extension"
              % __app_disp_name__)

    def get_widget(self, uri, window):
        """Returns a "crowbar" that will add a terminal in Dory.
        ...
        if uri.startswith("x-dory-desktop:///"):
            return
        #Event
    ...
    def _toggle_visible(self, window, event):
        """Toggle the visibility of Dory Terminal.

        This method is called on a "key-press-event" on the Dory'
        window.
    ...
    def get_name_and_desc(self):
        description = _("Embedded terminal for Dory")
        return [(f"dory-terminal:::{description}:::dory-terminal-prefs")]

if __name__ == "__main__":
    #Code for testing Dory Terminal outside of Dory
    print("%s %s\nOriginally By %s\nPorted for Dory by: %s"
          % (__app_disp_name__, __version__, __author__, __maintainer__))
    win = Gtk.Window()
    win.set_title("%s %s" % (__app_disp_name__, __version__))
    nterm = DoryTerminal("file://%s" % os.environ["HOME"], win)
