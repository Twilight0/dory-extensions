 ...
 *
 * dory-dropbox.c
 * Implements the Dory extension API for Dropbox. 
 *
...
gboolean dropbox_use_dory_submenu_workaround;
...
static void
reset_file(DoryFileInfo *file) {
  debug("resetting file %p", (void *) file);
  dory_file_info_invalidate_extension_info(file);
}

gboolean
reset_all_files(DoryDropbox *cvs) {
...
static void
when_file_dies(DoryDropbox *cvs, DoryFileInfo *address) {
  gchar *filename;
...
static void
changed_cb(DoryFileInfo *file, DoryDropbox *cvs) {
  ...
  uri = dory_file_info_get_uri(file);
  ...
  /* this is a hack, because dory doesn't do this for us, for some reason
...
dory_dropbox_update_file_info(DoryInfoProvider     *provider,
                                  DoryFileInfo         *file,
                                  GClosure                 *update_complete,
                                  DoryOperationHandle **handle) {
  DoryDropbox *cvs;

  cvs = DORY_DROPBOX(provider);
     ...
     so we can shell touch these files later */
  {
    ...
    g_free(uri);
    if (pfilename == NULL) {
      return DORY_OPERATION_COMPLETE;
    }
    else {
      int cmp = 0;
	...
	if (stored_filename != NULL && cmp != 0) {
	  ...
	  if ((f2 = g_hash_table_lookup(cvs->filename2obj, filename)) != NULL) {
	    /* if the filename exists in the filename2obj hash
	       ...
	       this happens when dory allocates another file object
...

  {
    ...
    *handle = (DoryOperationHandle *) dfic;
...
static void
handle_shell_touch(GHashTable *args, DoryDropbox *cvs) {
  gchar **path;
...
gboolean
dory_dropbox_finish_file_info_command(DropboxFileInfoCommandResponse *dficr) {

  //debug_enter();
  DoryOperationResult result = DORY_OPERATION_FAILED;

  if (!dficr->dfic->cancelled) {
    gchar **status = NULL;
    gboolean isdir;

    isdir = dory_file_info_is_directory(dficr->dfic->file) ;
	...
	(status = g_hash_table_lookup(dficr->emblems_response, "emblems")) != NULL) {
	  ...
	  if (status[i][0])
	    dory_file_info_add_emblem(dficr->dfic->file, status[i]);
      }
      result = DORY_OPERATION_COMPLETE;
    }
	...
	if (strcmp("public", tag[0]) == 0) {
	  dory_file_info_add_emblem(dficr->dfic->file, "web");
	}
	else if (strcmp("shared", tag[0]) == 0) {
	  dory_file_info_add_emblem(dficr->dfic->file, "people");
	}
	else if (strcmp("photos", tag[0]) == 0) {
	  dory_file_info_add_emblem(dficr->dfic->file, "photos");
	}
	else if (strcmp("sandbox", tag[0]) == 0) {
	  dory_file_info_add_emblem(dficr->dfic->file, "star");
	}
      }
	...
	if (emblem_code > 0) {
	  /*
	    ...
	    g_filename_from_uri(dory_file_info_get_uri(dficr->dfic->file),
	    NULL, NULL));
	  */
	  dory_file_info_add_emblem(dficr->dfic->file, emblems[emblem_code-1]);
	}
      }
      result = DORY_OPERATION_COMPLETE;
    }
  ...
  if (!dropbox_use_operation_in_progress_workaround) {
      dory_info_provider_update_complete_invoke(dficr->dfic->update_complete,
						    dficr->dfic->provider,
						    (DoryOperationHandle*) dficr->dfic,
						    result);
  }
...
static void
dory_dropbox_cancel_update(DoryInfoProvider     *provider,
                               DoryOperationHandle  *handle) {
...
static GList *
dory_dropbox_get_name_and_desc (DoryNameAndDescProvider *provider) {
    GList *ret = NULL;
    gchar *string = g_strdup_printf ("dory-dropbox:::%s",
...
static void
menu_item_cb(DoryMenuItem *item,
	     DoryDropbox *cvs) {
  gchar *verb;
  GList *files;
  ...
  files = g_object_get_data(G_OBJECT(item), "dory_dropbox_files");
  verb = g_object_get_data(G_OBJECT(item), "dory_dropbox_verb");
					     ...
					     (GDestroyNotify) g_strfreev);
  {
    ...
    for (li = files, i = 0; li != NULL; li = g_list_next(li)) {
      char *uri = dory_file_info_get_uri(DORY_FILE_INFO(li->data));
...
static int
dory_dropbox_parse_menu(gchar			**options,
			    DoryMenu		*menu,
			    ...
			    GList			*files)
{
  ...
  for ( i = 0; options[i] != NULL; i++) {
    ...
    if (strchr(item_inner, '~') != NULL) {
      ...
      DoryMenuItem *item;
      DoryMenu *submenu = dory_menu_new();

      g_string_append(new_action_string, item_name);
      g_string_append(new_action_string, "::");

      ret += dory_dropbox_parse_menu(suboptions, submenu, new_action_string,
					 toret, provider, files);

      item = dory_menu_item_new(new_action_string->str,
				    item_name, "", NULL);
      dory_menu_item_set_submenu(item, submenu);
      dory_menu_append_item(menu, item);
      ...
      item = dory_menu_item_new(new_action_string->str, item_name, item_inner, NULL);

      dory_menu_append_item(menu, item);
      /* add the file metadata to this item */
      g_object_set_data_full (G_OBJECT(item), "dory_dropbox_files",
			      dory_file_info_list_copy (files),
			      (GDestroyNotify) dory_file_info_list_free);
      /* add the verb metadata */
      g_object_set_data_full (G_OBJECT(item), "dory_dropbox_verb",
			      g_strdup(verb),
			      (GDestroyNotify) g_free);
	...
	g_object_set_property (G_OBJECT(item), "sensitive", &sensitive);
      ...
      /* taken from dory-file-repairer (http://repairer.kldp.net/):
       * this code is a workaround for a bug of dory
       * See: http://bugzilla.gnome.org/show_bug.cgi?id=508878 */
      if (dropbox_use_dory_submenu_workaround) {
...
static GList *
dory_dropbox_get_file_items(DoryMenuProvider *provider,
                                GtkWidget            *window,
				GList                *files)
{
  /*
  ...
  for (elem = files; elem; elem = elem->next, i++) {
    gchar *uri = dory_file_info_get_uri(elem->data);
   ...
   */
  DoryDropbox *cvs = DORY_DROPBOX(provider);
  dropbox_command_client_request(&(cvs->dc.dcc), (DropboxCommand *) dgc);

  GTimeVal gtv;

  /*
   * 4. We have to block until it's done because dory expects a reply.  But we will
  ...
  if (options && *options && **options)  {
    /* build the menu */
    DoryMenuItem *root_item;
    DoryMenu *root_menu;

    root_menu = dory_menu_new();
    root_item = dory_menu_item_new("DoryDropbox::root_item",
				       "Dropbox", "Dropbox Options", "dory-dropbox-symbolic");

    toret = g_list_append(toret, root_item);
    GString *action_string = g_string_new("DoryDropbox::");

    if (!dory_dropbox_parse_menu(options, root_menu, action_string,
				     toret, provider, files)) {
        g_list_free_full (toret, (GDestroyNotify) g_object_unref);
        toret = NULL;
    }

    dory_menu_item_set_submenu(root_item, root_menu);
...
add_emblem_paths(GHashTable* emblem_paths_response)
{
  ...
  gtk_icon_theme_append_search_path (theme, DORYICONDIR);
...
void get_emblem_paths_cb(GHashTable *emblem_paths_response, DoryDropbox *cvs)
{
  if (!emblem_paths_response) {
...
static void
on_connect(DoryDropbox *cvs) {
  reset_all_files(cvs);

  dropbox_command_client_send_command(&(cvs->dc.dcc),
				      (DoryDropboxCommandResponseHandler) get_emblem_paths_cb,
				      cvs, "get_emblem_paths", NULL);
}

static void
on_disconnect(DoryDropbox *cvs) {
  reset_all_files(cvs);
...
static void
dory_dropbox_menu_provider_iface_init (DoryMenuProviderIface *iface) {
  iface->get_file_items = dory_dropbox_get_file_items;
  return;
}

static void
dory_dropbox_info_provider_iface_init (DoryInfoProviderIface *iface) {
  iface->update_file_info = dory_dropbox_update_file_info;
  iface->cancel_update = dory_dropbox_cancel_update;
  return;
}

static void
dory_dropbox_nd_provider_iface_init (DoryNameAndDescProviderIface *iface) {
  iface->get_name_and_desc = dory_dropbox_get_name_and_desc;
}

static void
dory_dropbox_instance_init (DoryDropbox *cvs) {
  ...
  /* our hooks */
  dory_dropbox_hooks_add(&(cvs->dc.hookserv), "shell_touch",
...
static void
dory_dropbox_class_init (DoryDropboxClass *class) {
}

static void
dory_dropbox_class_finalize (DoryDropboxClass *class) {
  debug("just checking");
  /* kill threads here? */
}

GType
dory_dropbox_get_type (void) {
  return dropbox_type;
}

void
dory_dropbox_register_type (GTypeModule *module) {
  static const GTypeInfo info = {
    sizeof (DoryDropboxClass),
    (GBaseInitFunc) NULL,
    (GBaseFinalizeFunc) NULL,
    (GClassInitFunc) dory_dropbox_class_init,
    (GClassFinalizeFunc) dory_dropbox_class_finalize,
    NULL,
    sizeof (DoryDropbox),
    0,
    (GInstanceInitFunc) dory_dropbox_instance_init,
  };

  static const GInterfaceInfo menu_provider_iface_info = {
    (GInterfaceInitFunc) dory_dropbox_menu_provider_iface_init,
    NULL,
    NULL
  };

  static const GInterfaceInfo info_provider_iface_info = {
    (GInterfaceInitFunc) dory_dropbox_info_provider_iface_init,
    NULL,
    NULL
  };

  static const GInterfaceInfo nd_provider_iface_info = {
    (GInterfaceInitFunc) dory_dropbox_nd_provider_iface_init,
    NULL,
    NULL
  };
