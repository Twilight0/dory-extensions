 ...
 * Header file for dory-dropbox-command.c
 *
...
/* command structs */
typedef enum {GET_FILE_INFO, GENERAL_COMMAND} DoryDropboxRequestType;

typedef struct {
  DoryDropboxRequestType request_type;
} DropboxCommand;
...
typedef void (*DoryDropboxCommandResponseHandler)(GHashTable *, gpointer);

typedef struct {
  DropboxCommand dc;
  gchar *command_name;
  GHashTable *command_args;
  DoryDropboxCommandResponseHandler handler;
  gpointer handler_ud;
