...
static gboolean
try_to_connect(DoryDropboxHookserv *hookserv);

static gboolean
handle_hook_server_input(GIOChannel *chan,
			 GIOCondition cond,
			 DoryDropboxHookserv *hookserv) {
  /*debug_enter(); */
...
static void
watch_killer(DoryDropboxHookserv *hookserv) {
...
gboolean dory_dropbox_hooks_force_reconnect(DoryDropboxHookserv *hookserv) {
  debug_enter();
...
gboolean
dory_dropbox_hooks_is_connected(DoryDropboxHookserv *hookserv) {
  return hookserv->connected;
}

void
dory_dropbox_hooks_setup(DoryDropboxHookserv *hookserv) {
...
void
dory_dropbox_hooks_add_on_disconnect_hook(DoryDropboxHookserv *hookserv,
...
void
dory_dropbox_hooks_add_on_connect_hook(DoryDropboxHookserv *hookserv,
					   DropboxHookClientConnectHook dhcch,
					   gpointer ud) {
  GHook *newhook;
...
void dory_dropbox_hooks_add(DoryDropboxHookserv *ndhs,
...
void
dory_dropbox_hooks_start(DoryDropboxHookserv *hookserv) {
  try_to_connect(hookserv);
}
