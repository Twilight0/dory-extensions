#!/usr/bin/python3
# -*- coding: utf-8 -*-
#    dory-compare --- Context menu extension for Dory file manager
...
from gi.repository import Dory, GObject, Gio

sys.path.append("/usr/share/dory-compare")

import utils

class DoryCompareExtension(GObject.GObject, Dory.MenuProvider, Dory.NameAndDescProvider):
    ...
    def __init__(self):
        ...
        self.config = utils.DoryCompareConfig()
        self.config.load()
    ...
    def get_file_items(self, window, files):
        ...
        if len(paths) == 1:
            ...
            if self.for_later is not None:
                ...
                if self.for_later not in paths:
                    item1 = Dory.MenuItem(
                        name="DoryCompareExtension::CompareTo",
            ...
            item3 = Dory.MenuItem(
                name="DoryCompareExtension::CompareLater",
        ...
        # can always compare, if more than one selected
            ...
            if self.for_later is not None:
                if self.for_later not in paths:
                    ...
                    if len(self.config.diff_engine_multi.strip()) > 0 or (len(paths) == 2 and len(self.config.diff_engine_3way.strip()) > 0):
                        item1 = Dory.MenuItem(
                            name="DoryCompareExtension::MultiCompare",
                            label=_('Compare to: ') + for_later_relative,
                            tip=_("Compare to the file previously selected")
                        )
            ...
            if len(self.config.diff_engine_multi.strip()) > 0 or len(paths) == 2 or (len(paths) == 3 and len(self.config.diff_engine_3way.strip()) > 0):
                item2 = Dory.MenuItem(
                    name="DoryCompareExtension::CompareWithin",
                    label=_('Compare'),
    ...
    def get_name_and_desc(self):
        ...
        return [(f"dory-compare:::{description}:::dory-compare-preferences")]
