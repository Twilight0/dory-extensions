...
#include "dory-preview-cover-art.h"

#include <musicbrainz5/mb5_c.h>
#include <gdk-pixbuf/gdk-pixbuf.h>

G_DEFINE_TYPE (DoryPreviewCoverArtFetcher, dory_preview_cover_art_fetcher, G_TYPE_OBJECT);

#define DORY_PREVIEW_COVER_ART_FETCHER_GET_PRIVATE(obj)\
  (G_TYPE_INSTANCE_GET_PRIVATE ((obj), DORY_PREVIEW_TYPE_COVER_ART_FETCHER, DoryPreviewCoverArtFetcherPrivate))
...
struct _DoryPreviewCoverArtFetcherPrivate {
  GdkPixbuf *cover;
  GstTagList *taglist;
...
static void dory_preview_cover_art_fetcher_set_taglist (DoryPreviewCoverArtFetcher *self,
                                                 GstTagList *taglist);
static void dory_preview_cover_art_fetcher_get_uri_for_track_async (DoryPreviewCoverArtFetcher *self,
                                                             const gchar *artist,
                                                             const gchar *album,
                                                             GAsyncReadyCallback callback,
                                                             gpointer user_data);
static void try_read_from_file (DoryPreviewCoverArtFetcher *self,
                                GFile *file);

static void
dory_preview_cover_art_fetcher_dispose (GObject *object)
{
  DoryPreviewCoverArtFetcherPrivate *priv = DORY_PREVIEW_COVER_ART_FETCHER_GET_PRIVATE (object);
  ...
  G_OBJECT_CLASS (dory_preview_cover_art_fetcher_parent_class)->dispose (object);
}

static void
dory_preview_cover_art_fetcher_get_property (GObject    *gobject,
                                      guint       prop_id,
                                      GValue     *value,
                                      GParamSpec *pspec)
{
  DoryPreviewCoverArtFetcher *self = DORY_PREVIEW_COVER_ART_FETCHER (gobject);
  DoryPreviewCoverArtFetcherPrivate *priv = DORY_PREVIEW_COVER_ART_FETCHER_GET_PRIVATE (self);
...
static void
dory_preview_cover_art_fetcher_set_property (GObject    *gobject,
                                      guint       prop_id,
                                      const GValue *value,
                                      GParamSpec *pspec)
{
  ...
  switch (prop_id) {
  case PROP_TAGLIST:
    dory_preview_cover_art_fetcher_set_taglist (self, g_value_get_boxed (value));
    break;
  default:
    G_OBJECT_WARN_INVALID_PROPERTY_ID (gobject, prop_id, pspec);
    break;
...
static void
dory_preview_cover_art_fetcher_init (DoryPreviewCoverArtFetcher *self)
{
  self->priv = DORY_PREVIEW_COVER_ART_FETCHER_GET_PRIVATE (self);
}

static void
dory_preview_cover_art_fetcher_class_init (DoryPreviewCoverArtFetcherClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->get_property = dory_preview_cover_art_fetcher_get_property;
  gobject_class->set_property = dory_preview_cover_art_fetcher_set_property;
  gobject_class->dispose = dory_preview_cover_art_fetcher_dispose;
  ...
  g_type_class_add_private (klass, sizeof (DoryPreviewCoverArtFetcherPrivate));
}

typedef struct {
  DoryPreviewCoverArtFetcher *self;
...
static FetchUriJob *
fetch_uri_job_new (DoryPreviewCoverArtFetcher *self,
                   const gchar *artist,
                   const gchar *album,
                   GAsyncReadyCallback callback,
                   gpointer user_data)
{
  ...
  retval->result = g_simple_async_result_new (G_OBJECT (self), callback, user_data,
                                              dory_preview_cover_art_fetcher_get_uri_for_track_async);
...
static gchar *
dory_preview_cover_art_fetcher_get_uri_for_track_finish (DoryPreviewCoverArtFetcher *self,
                                                  GAsyncResult *result,
                                                  GError **error)
{
  gchar *retval;
...
static void
dory_preview_cover_art_fetcher_get_uri_for_track_async (DoryPreviewCoverArtFetcher *self,
                                                 const gchar *artist,
                                                 const gchar *album,
                                                 GAsyncReadyCallback callback,
                                                 gpointer user_data)
...
static GFile *
get_gfile_for_amazon (DoryPreviewCoverArtFetcher *self)
{
  GFile *retval;
  gchar *uri;
...
static GFile *
get_gfile_for_cache (DoryPreviewCoverArtFetcher *self)
{
  GFile *retval;
  gchar *cache_path;
  gchar *filename, *path;
                        ...
                        gpointer user_data)
{
  ...
  GError *error = NULL;
  DoryPreviewCoverArtFetcher *self = user_data;
...
static void
try_read_from_file (DoryPreviewCoverArtFetcher *self,
                    GFile *file)
{
...
amazon_cover_uri_async_ready_cb (GObject *source,
                                 ...
                                 gpointer user_data)
{
  DoryPreviewCoverArtFetcher *self = DORY_PREVIEW_COVER_ART_FETCHER (source);
  GError *error = NULL;
  GFile *file;

  self->priv->asin = dory_preview_cover_art_fetcher_get_uri_for_track_finish
    (self, res, &error);
...
static void
try_fetch_from_amazon (DoryPreviewCoverArtFetcher *self)
{
  DoryPreviewCoverArtFetcherPrivate *priv = DORY_PREVIEW_COVER_ART_FETCHER_GET_PRIVATE (self);
  gchar *artist = NULL;
  gchar *album = NULL;
...
/* */
static void
try_fetch_from_tags (DoryPreviewCoverArtFetcher *self)
{
  DoryPreviewCoverArtFetcherPrivate *priv = DORY_PREVIEW_COVER_ART_FETCHER_GET_PRIVATE (self);
...
static void
dory_preview_cover_art_fetcher_set_taglist (DoryPreviewCoverArtFetcher *self,
                                     GstTagList *taglist)
{
  DoryPreviewCoverArtFetcherPrivate *priv = DORY_PREVIEW_COVER_ART_FETCHER_GET_PRIVATE (self);
...
DoryPreviewCoverArtFetcher *
dory_preview_cover_art_fetcher_new (GstTagList *taglist)
{
  return g_object_new (DORY_PREVIEW_TYPE_COVER_ART_FETCHER,
                       "taglist", taglist,
                       NULL);
}
