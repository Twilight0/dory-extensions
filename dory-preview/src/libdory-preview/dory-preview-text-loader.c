...
#include "dory-preview-text-loader.h"
...
G_DEFINE_TYPE (DoryPreviewTextLoader, dory_preview_text_loader, G_TYPE_OBJECT);
...
struct _DoryPreviewTextLoaderPrivate {
  gchar *uri;
...
static GtkSourceLanguage *
text_loader_get_buffer_language (DoryPreviewTextLoader *self,
                                 GFile *file)
{
...
load_contents_async_ready_cb (GObject *source,
                              ...
                              gpointer user_data)
{
  DoryPreviewTextLoader *self = user_data;
  GError *error = NULL;
  gchar *contents;
  GtkSourceLanguage *language = NULL;
...
static void
start_loading_buffer (DoryPreviewTextLoader *self)
{
  GFile *file;
...
static void
dory_preview_text_loader_set_uri (DoryPreviewTextLoader *self,
                          const gchar *uri)
{
...
static void
dory_preview_text_loader_dispose (GObject *object)
{
  DoryPreviewTextLoader *self = DORY_PREVIEW_TEXT_LOADER (object);

  g_free (self->priv->uri);

  G_OBJECT_CLASS (dory_preview_text_loader_parent_class)->dispose (object);
}

static void
dory_preview_text_loader_get_property (GObject *object,
                               guint       prop_id,
                               GValue     *value,
                               GParamSpec *pspec)
{
...
static void
dory_preview_text_loader_set_property (GObject *object,
                               guint       prop_id,
                               const GValue *value,
                               GParamSpec *pspec)
{
  ...
  switch (prop_id) {
  case PROP_URI:
    dory_preview_text_loader_set_uri (self, g_value_get_string (value));
    break;
  default:
    G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
    break;
...
static void
dory_preview_text_loader_class_init (DoryPreviewTextLoaderClass *klass)
{
  GObjectClass *oclass;

  oclass = G_OBJECT_CLASS (klass);
  oclass->dispose = dory_preview_text_loader_dispose;
  oclass->get_property = dory_preview_text_loader_get_property;
  oclass->set_property = dory_preview_text_loader_set_property;
  ...
  g_type_class_add_private (klass, sizeof (DoryPreviewTextLoaderPrivate));
}

static void
dory_preview_text_loader_init (DoryPreviewTextLoader *self)
{
  self->priv =
    G_TYPE_INSTANCE_GET_PRIVATE (self,
                                 DORY_PREVIEW_TYPE_TEXT_LOADER,
                                 DoryPreviewTextLoaderPrivate);
}

DoryPreviewTextLoader *
dory_preview_text_loader_new (const gchar *uri)
{
  return g_object_new (DORY_PREVIEW_TYPE_TEXT_LOADER,
                       "uri", uri,
                       NULL);
}
