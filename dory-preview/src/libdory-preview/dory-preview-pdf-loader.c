...
#include "dory-preview-pdf-loader.h"

#include "dory-preview-utils.h"
#include <xreader-document.h>
#include <xreader-view.h>
#include <glib/gstdio.h>
#include <gdk/gdkx.h>

G_DEFINE_TYPE (DoryPreviewPdfLoader, dory_preview_pdf_loader, G_TYPE_OBJECT);
...
static void load_libreoffice (DoryPreviewPdfLoader *self);

struct _DoryPreviewPdfLoaderPrivate {
  EvDocument *document;
  gchar *uri;
  gchar *pdf_path;
...
load_job_done (EvJob *job,
               gpointer user_data)
{
  DoryPreviewPdfLoader *self = user_data;
...
static void
load_pdf (DoryPreviewPdfLoader *self,
          const gchar *uri)
{
  EvJob *job;
...
static void
libreoffice_missing (DoryPreviewPdfLoader *self)
{
...
static void
load_libreoffice (DoryPreviewPdfLoader *self)
{
...
content_type_is_native (const gchar *content_type)
{
  ...
  native_types = dory_preview_query_supported_document_types ();
...
static void
start_loading_document (DoryPreviewPdfLoader *self)
{
  GFile *file;
...
static void
dory_preview_pdf_loader_set_uri (DoryPreviewPdfLoader *self,
                          const gchar *uri)
{
...
void
dory_preview_pdf_loader_cleanup_document (DoryPreviewPdfLoader *self)
{
...
static void
dory_preview_pdf_loader_dispose (GObject *object)
{
  DoryPreviewPdfLoader *self = DORY_PREVIEW_PDF_LOADER (object);

  dory_preview_pdf_loader_cleanup_document (self);

  g_clear_object (&self->priv->document);
  g_free (self->priv->uri);

  G_OBJECT_CLASS (dory_preview_pdf_loader_parent_class)->dispose (object);
}

static void
dory_preview_pdf_loader_get_property (GObject *object,
                               guint       prop_id,
                               GValue     *value,
                               GParamSpec *pspec)
{
...
static void
dory_preview_pdf_loader_set_property (GObject *object,
                               guint       prop_id,
                               const GValue *value,
                               GParamSpec *pspec)
{
  ...
  switch (prop_id) {
  case PROP_URI:
    dory_preview_pdf_loader_set_uri (self, g_value_get_string (value));
    break;
  default:
    G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
    break;
...
static void
dory_preview_pdf_loader_class_init (DoryPreviewPdfLoaderClass *klass)
{
  GObjectClass *oclass;

  oclass = G_OBJECT_CLASS (klass);
  oclass->dispose = dory_preview_pdf_loader_dispose;
  oclass->get_property = dory_preview_pdf_loader_get_property;
  oclass->set_property = dory_preview_pdf_loader_set_property;
    ...
    g_type_class_add_private (klass, sizeof (DoryPreviewPdfLoaderPrivate));
}

static void
dory_preview_pdf_loader_init (DoryPreviewPdfLoader *self)
{
  self->priv =
    G_TYPE_INSTANCE_GET_PRIVATE (self,
                                 DORY_PREVIEW_TYPE_PDF_LOADER,
                                 DoryPreviewPdfLoaderPrivate);
  self->priv->libreoffice_pid = -1;
}

DoryPreviewPdfLoader *
dory_preview_pdf_loader_new (const gchar *uri)
{
  return g_object_new (DORY_PREVIEW_TYPE_PDF_LOADER,
                       "uri", uri,
                       NULL);
}

/**
 * dory_preview_pdf_loader_get_max_page_size:
 * @self:
 * @width: (out):
 * @height: (out):
 *
 */
void
dory_preview_pdf_loader_get_max_page_size (DoryPreviewPdfLoader *self,
                                    gdouble *width,
                                    gdouble *height)
{
