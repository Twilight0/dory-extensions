...
#include "dory-preview-file-loader.h"
...
G_DEFINE_TYPE (DoryPreviewFileLoader, dory_preview_file_loader, G_TYPE_OBJECT);
...
struct _DoryPreviewFileLoaderPrivate {
  GFile *file;
  GFileInfo *info;
...
{
  DoryPreviewFileLoader *self = user_data;
...
static void
queue_size_notify (DoryPreviewFileLoader *self)
{
...
static void
deep_count_start (DoryPreviewFileLoader *self)
{
  DeepCountState *state;
...
static void
start_loading_file (DoryPreviewFileLoader *self)
{
...
static void
dory_preview_file_loader_set_file (DoryPreviewFileLoader *self,
                            GFile *file)
{
...
static void
dory_preview_file_loader_dispose (GObject *object)
{
  DoryPreviewFileLoader *self = DORY_PREVIEW_FILE_LOADER (object);
  ...
  G_OBJECT_CLASS (dory_preview_file_loader_parent_class)->dispose (object);
}

static void
dory_preview_file_loader_get_property (GObject *object,
                                guint       prop_id,
                                GValue     *value,
                                GParamSpec *pspec)
{
  ...
  switch (prop_id) {
  case PROP_NAME:
    g_value_take_string (value, dory_preview_file_loader_get_display_name (self));
    break;
  case PROP_SIZE:
    g_value_take_string (value, dory_preview_file_loader_get_size_string (self));
    break;
  case PROP_TIME:
    g_value_take_string (value, dory_preview_file_loader_get_date_string (self));
    break;
  case PROP_ICON:
    g_value_take_object (value, dory_preview_file_loader_get_icon (self));
    break;
  case PROP_FILE:
    g_value_set_object (value, self->priv->file);
    break;
  case PROP_CONTENT_TYPE:
    g_value_take_string (value, dory_preview_file_loader_get_content_type_string (self));
    break;
  case PROP_FILE_TYPE:
    g_value_set_enum (value, dory_preview_file_loader_get_file_type (self));
    break;
  default:
...
static void
dory_preview_file_loader_set_property (GObject *object,
                                guint       prop_id,
                                const GValue *value,
                                GParamSpec *pspec)
{
  ...
  switch (prop_id) {
  case PROP_FILE:
    dory_preview_file_loader_set_file (self, g_value_get_object (value));
    break;
  default:
    G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
    break;
...
static void
dory_preview_file_loader_class_init (DoryPreviewFileLoaderClass *klass)
{
  GObjectClass *oclass;

  oclass = G_OBJECT_CLASS (klass);
  oclass->dispose = dory_preview_file_loader_dispose;
  oclass->get_property = dory_preview_file_loader_get_property;
  oclass->set_property = dory_preview_file_loader_set_property;
  ...
  g_type_class_add_private (klass, sizeof (DoryPreviewFileLoaderPrivate));
  g_object_class_install_properties (oclass, NUM_PROPERTIES, properties);
}

static void
dory_preview_file_loader_init (DoryPreviewFileLoader *self)
{
  self->priv =
    G_TYPE_INSTANCE_GET_PRIVATE (self,
                                 DORY_PREVIEW_TYPE_FILE_LOADER,
                                 DoryPreviewFileLoaderPrivate);

  self->priv->cancellable = g_cancellable_new ();
  self->priv->total_size = -1;
}

DoryPreviewFileLoader *
dory_preview_file_loader_new (GFile *file)
{
  return g_object_new (DORY_PREVIEW_TYPE_FILE_LOADER,
                       "file", file,
                       NULL);
}

/**
 * dory_preview_file_loader_get_display_name:
 * @self:
 *
 * Returns: (transfer full):
 */
gchar *
dory_preview_file_loader_get_display_name (DoryPreviewFileLoader *self)
{
...
/**
 * dory_preview_file_loader_get_icon:
 * @self:
 *
 * Returns: (transfer full):
 */
GdkPixbuf *
dory_preview_file_loader_get_icon (DoryPreviewFileLoader *self)
{
  GdkPixbuf *retval;
  GtkIconInfo *info;
  GError *error = NULL;
...
/**
 * dory_preview_file_loader_get_size_string:
 * @self:
 *
 * Returns: (transfer full):
 */
gchar *
dory_preview_file_loader_get_size_string (DoryPreviewFileLoader *self)
{
  goffset size;
...
gboolean
dory_preview_file_loader_get_loading (DoryPreviewFileLoader *self)
{
  return self->priv->loading;
}

/**
 * dory_preview_file_loader_get_date_string:
 * @self:
 *
 * Returns: (transfer full):
 */
gchar *
dory_preview_file_loader_get_date_string (DoryPreviewFileLoader *self)
{
  GTimeVal timeval;
  GDateTime *date;
  gchar *retval;
...
/**
 * dory_preview_file_loader_get_content_type_string:
 * @self:
 *
 * Returns: (transfer full):
 */
gchar *
dory_preview_file_loader_get_content_type_string (DoryPreviewFileLoader *self)
{
  if (self->priv->info == NULL)
    return NULL;
...
/**
 * dory_preview_file_loader_get_file_type:
 * @self:
 *
 * Returns:
 */
GFileType
dory_preview_file_loader_get_file_type (DoryPreviewFileLoader *self)
{
  if (self->priv->info == NULL)
...
void
dory_preview_file_loader_stop (DoryPreviewFileLoader *self)
{
  if (self->priv->cancellable == NULL)
    return;
