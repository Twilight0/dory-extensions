...
#include "dory-preview-font-widget.h"
#include "dory-preview-font-loader.h"
...
struct _DoryPreviewFontWidgetPrivate {
  gchar *uri;
...
G_DEFINE_TYPE (DoryPreviewFontWidget, dory_preview_font_widget, GTK_TYPE_DRAWING_AREA);
 ...
 */
static void
draw_string (DoryPreviewFontWidget *self,
             cairo_t *cr,
             GtkBorder padding,
	     const gchar *text,
	     gint *pos_y)
...
static gboolean
set_pango_sample_string (DoryPreviewFontWidget *self)
{
...
static void
build_strings_for_face (DoryPreviewFontWidget *self)
{
...
static void
dory_preview_font_widget_size_request (GtkWidget *drawing_area,
                                gint *width,
                                gint *height,
                                gint *min_height)
{
  DoryPreviewFontWidget *self = DORY_PREVIEW_FONT_WIDGET (drawing_area);
  DoryPreviewFontWidgetPrivate *priv = self->priv;
...
static void
dory_preview_font_widget_get_preferred_width (GtkWidget *drawing_area,
                                       gint *minimum_width,
                                       gint *natural_width)
{
  gint width;

  dory_preview_font_widget_size_request (drawing_area, &width, NULL, NULL);

  *minimum_width = *natural_width = width;
}

static void
dory_preview_font_widget_get_preferred_height (GtkWidget *drawing_area,
                                        gint *minimum_height,
                                        gint *natural_height)
{
  gint height, min_height;

  dory_preview_font_widget_size_request (drawing_area, NULL, &height, &min_height);
...
static gboolean
dory_preview_font_widget_draw (GtkWidget *drawing_area,
                        cairo_t *cr)
{
  DoryPreviewFontWidget *self = DORY_PREVIEW_FONT_WIDGET (drawing_area);
  DoryPreviewFontWidgetPrivate *priv = self->priv;
...
font_face_async_ready_cb (GObject *object,
                          ...
                          gpointer user_data)
{
  DoryPreviewFontWidget *self = user_data;
  GError *error = NULL;

  self->priv->face =
    dory_preview_new_ft_face_from_uri_finish (result,
...
static void
load_font_face (DoryPreviewFontWidget *self)
{
  dory_preview_new_ft_face_from_uri_async (self->priv->library,
                                    self->priv->uri,
...
static void
dory_preview_font_widget_set_uri (DoryPreviewFontWidget *self,
                           const gchar *uri)
{
...
static void
dory_preview_font_widget_init (DoryPreviewFontWidget *self)
{
  FT_Error err;

  self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, DORY_PREVIEW_TYPE_FONT_WIDGET,
                                            DoryPreviewFontWidgetPrivate);
...
static void
dory_preview_font_widget_get_property (GObject *object,
                                guint       prop_id,
                                GValue     *value,
                                GParamSpec *pspec)
{
  DoryPreviewFontWidget *self = DORY_PREVIEW_FONT_WIDGET (object);
...
static void
dory_preview_font_widget_set_property (GObject *object,
                               guint       prop_id,
                               const GValue *value,
                               GParamSpec *pspec)
{
  ...
  switch (prop_id) {
  case PROP_URI:
    dory_preview_font_widget_set_uri (self, g_value_get_string (value));
    break;
  default:
    G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
    break;
...
static void
dory_preview_font_widget_finalize (GObject *object)
{
  DoryPreviewFontWidget *self = DORY_PREVIEW_FONT_WIDGET (object);
  ...
  G_OBJECT_CLASS (dory_preview_font_widget_parent_class)->finalize (object);
}

static void
dory_preview_font_widget_class_init (DoryPreviewFontWidgetClass *klass)
{
  GObjectClass *oclass = G_OBJECT_CLASS (klass);
  GtkWidgetClass *wclass = GTK_WIDGET_CLASS (klass);

  oclass->finalize = dory_preview_font_widget_finalize;
  oclass->set_property = dory_preview_font_widget_set_property;
  oclass->get_property = dory_preview_font_widget_get_property;

  wclass->draw = dory_preview_font_widget_draw;
  wclass->get_preferred_width = dory_preview_font_widget_get_preferred_width;
  wclass->get_preferred_height = dory_preview_font_widget_get_preferred_height;
  ...
  g_type_class_add_private (klass, sizeof (DoryPreviewFontWidgetPrivate));
}

DoryPreviewFontWidget *
dory_preview_font_widget_new (const gchar *uri)
{
  return g_object_new (DORY_PREVIEW_TYPE_FONT_WIDGET,
                       "uri", uri,
                       NULL);
}

/**
 * dory_preview_font_widget_get_ft_face: (skip)
 *
 */
FT_Face
dory_preview_font_widget_get_ft_face (DoryPreviewFontWidget *self)
{
  return self->priv->face;
}

const gchar *
dory_preview_font_widget_get_uri (DoryPreviewFontWidget *self)
{
