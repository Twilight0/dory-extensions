...
typedef struct _DoryImageRotatorPrivate DoryImageRotatorPrivate;

struct _DoryImageRotatorPrivate {
	GList *files;
...
#define DORY_IMAGE_ROTATOR_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), DORY_TYPE_IMAGE_ROTATOR, DoryImageRotatorPrivate))

G_DEFINE_TYPE (DoryImageRotator, dory_image_rotator, G_TYPE_OBJECT)
...
typedef enum {
	...
	SIGNAL_TYPE_EXAMPLE,
	LAST_SIGNAL
} DoryImageRotatorSignalType;

static void
dory_image_rotator_finalize(GObject *object)
{
	DoryImageRotator *dialog = DORY_IMAGE_ROTATOR (object);
	DoryImageRotatorPrivate *priv = DORY_IMAGE_ROTATOR_GET_PRIVATE (dialog);
	
	g_free (priv->suffix);
		
	G_OBJECT_CLASS(dory_image_rotator_parent_class)->finalize(object);
}

static void
dory_image_rotator_set_property (GObject      *object,
                        guint         property_id,
                        const GValue *value,
                        GParamSpec   *pspec)
{
...
static void
dory_image_rotator_get_property (GObject      *object,
                        guint         property_id,
                        GValue       *value,
                        GParamSpec   *pspec)
{
	DoryImageRotator *self = DORY_IMAGE_ROTATOR (object);
	DoryImageRotatorPrivate *priv = DORY_IMAGE_ROTATOR_GET_PRIVATE (self);
...
static void
dory_image_rotator_class_init(DoryImageRotatorClass *klass)
{
	g_type_class_add_private (klass, sizeof (DoryImageRotatorPrivate));

	GObjectClass *object_class = G_OBJECT_CLASS(klass);
	GParamSpec *files_param_spec;

	object_class->finalize = dory_image_rotator_finalize;
	object_class->set_property = dory_image_rotator_set_property;
	object_class->get_property = dory_image_rotator_get_property;
...
static void run_op (DoryImageRotator *rotator);

static GFile *
dory_image_rotator_transform_filename (DoryImageRotator *rotator, GFile *orig_file)
{
	DoryImageRotatorPrivate *priv = DORY_IMAGE_ROTATOR_GET_PRIVATE (rotator);
...
{
	DoryImageRotator *rotator = DORY_IMAGE_ROTATOR (data);
	DoryImageRotatorPrivate *priv = DORY_IMAGE_ROTATOR_GET_PRIVATE (rotator);
	
	gboolean retry = TRUE;
	
	DoryFileInfo *file = DORY_FILE_INFO (priv->files->data);
	
	if (status != 0) {
		/* rotating failed */
		char *name = dory_file_info_get_name (file);
		...
		GFile *orig_location = dory_file_info_get_location (file);
		GFile *new_location = dory_image_rotator_transform_filename (rotator, orig_location);
...
static void
run_op (DoryImageRotator *rotator)
{
	DoryImageRotatorPrivate *priv = DORY_IMAGE_ROTATOR_GET_PRIVATE (rotator);
...
static void
dory_image_rotator_response_cb (GtkDialog *dialog, gint response_id, gpointer user_data)
{
	DoryImageRotator *rotator = DORY_IMAGE_ROTATOR (user_data);
	DoryImageRotatorPrivate *priv = DORY_IMAGE_ROTATOR_GET_PRIVATE (rotator);
...
static void
dory_image_rotator_init(DoryImageRotator *rotator)
{
	DoryImageRotatorPrivate *priv = DORY_IMAGE_ROTATOR_GET_PRIVATE (rotator);
	...
	path = g_build_filename (DATADIR, PACKAGE, "dory-image-rotate.ui", NULL);
	...
	g_signal_connect (G_OBJECT (priv->rotate_dialog), "response",
			  (GCallback) dory_image_rotator_response_cb,
			  rotator);
}

DoryImageRotator *
dory_image_rotator_new (GList *files)
{
	return g_object_new (DORY_TYPE_IMAGE_ROTATOR, "files", files, NULL);
}

void
dory_image_rotator_show_dialog (DoryImageRotator *rotator)
{
	DoryImageRotatorPrivate *priv = DORY_IMAGE_ROTATOR_GET_PRIVATE (rotator);
