...
typedef struct _DoryImageResizerPrivate DoryImageResizerPrivate;

struct _DoryImageResizerPrivate {
	GList *files;
...
#define DORY_IMAGE_RESIZER_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), DORY_TYPE_IMAGE_RESIZER, DoryImageResizerPrivate))

G_DEFINE_TYPE (DoryImageResizer, dory_image_resizer, G_TYPE_OBJECT)
...
typedef enum {
	...
	SIGNAL_TYPE_EXAMPLE,
	LAST_SIGNAL
} DoryImageResizerSignalType;

static void
dory_image_resizer_finalize(GObject *object)
{
	DoryImageResizer *dialog = DORY_IMAGE_RESIZER (object);
	DoryImageResizerPrivate *priv = DORY_IMAGE_RESIZER_GET_PRIVATE (dialog);
	
	g_free (priv->suffix);
		
	G_OBJECT_CLASS(dory_image_resizer_parent_class)->finalize(object);
}

static void
dory_image_resizer_set_property (GObject      *object,
                        guint         property_id,
                        const GValue *value,
                        GParamSpec   *pspec)
{
...
static void
dory_image_resizer_get_property (GObject      *object,
                        guint         property_id,
                        GValue       *value,
                        GParamSpec   *pspec)
{
	DoryImageResizer *self = DORY_IMAGE_RESIZER (object);
	DoryImageResizerPrivate *priv = DORY_IMAGE_RESIZER_GET_PRIVATE (self);
...
static void
dory_image_resizer_class_init(DoryImageResizerClass *klass)
{
	g_type_class_add_private (klass, sizeof (DoryImageResizerPrivate));

	GObjectClass *object_class = G_OBJECT_CLASS(klass);
	GParamSpec *files_param_spec;

	object_class->finalize = dory_image_resizer_finalize;
	object_class->set_property = dory_image_resizer_set_property;
	object_class->get_property = dory_image_resizer_get_property;
...
static void run_op (DoryImageResizer *resizer);

static GFile *
dory_image_resizer_transform_filename (DoryImageResizer *resizer, GFile *orig_file)
{
	DoryImageResizerPrivate *priv = DORY_IMAGE_RESIZER_GET_PRIVATE (resizer);
...
{
	DoryImageResizer *resizer = DORY_IMAGE_RESIZER (data);
	DoryImageResizerPrivate *priv = DORY_IMAGE_RESIZER_GET_PRIVATE (resizer);
	...
	if (status != 0) {
		...
		GFile *new_location = dory_image_resizer_transform_filename (resizer, orig_location);
...
static void
run_op (DoryImageResizer *resizer)
{
	DoryImageResizerPrivate *priv = DORY_IMAGE_RESIZER_GET_PRIVATE (resizer);
...
static void
dory_image_resizer_response_cb (GtkDialog *dialog, gint response_id, gpointer user_data)
{
	DoryImageResizer *resizer = DORY_IMAGE_RESIZER (user_data);
	DoryImageResizerPrivate *priv = DORY_IMAGE_RESIZER_GET_PRIVATE (resizer);
...
static void
dory_image_resizer_init(DoryImageResizer *resizer)
{
	DoryImageResizerPrivate *priv = DORY_IMAGE_RESIZER_GET_PRIVATE (resizer);
	...
	path = g_build_filename (DATADIR, PACKAGE, "dory-image-resize.ui", NULL);
	...
	g_signal_connect (G_OBJECT (priv->resize_dialog), "response",
			  (GCallback) dory_image_resizer_response_cb,
			  resizer);
}

DoryImageResizer *
dory_image_resizer_new (GList *files)
{
	return g_object_new (DORY_TYPE_IMAGE_RESIZER, "files", files, NULL);
}

void
dory_image_resizer_show_dialog (DoryImageResizer *resizer)
{
	DoryImageResizerPrivate *priv = DORY_IMAGE_RESIZER_GET_PRIVATE (resizer);
