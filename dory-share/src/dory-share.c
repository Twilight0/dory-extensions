...
typedef struct {
  ...
  DoryFileInfo *fileinfo; /* Dory file to which this page refers */
...
message_confirm_missing_permissions (GtkWidget *widget, const char *path, mode_t need_mask)
{
  ...
  dialog = gtk_message_dialog_new (toplevel ? GTK_WINDOW (toplevel) : NULL,
				   ...
				   GTK_BUTTONS_NONE,
				   _("Dory needs to add some permissions to your folder \"%s\" in order to share it"),
				   display_name);
  ...
  gtk_message_dialog_format_secondary_text (GTK_MESSAGE_DIALOG (dialog),
					    _("The folder \"%s\" needs the following extra permissions for sharing to work:\n"
					      "%s%s%s"
					      "Do you want Dory to add these permissions to the folder automatically?"),
					    display_name,
...
{
  return g_build_filename (g_get_home_dir (), ".gnome2", "dory-share-modified-permissions", NULL);
}
...
property_page_commit (PropertyPage *page)
{
  ...
  if (!retval)
    ...
    {
      dory_file_info_invalidate_extension_info (page->fileinfo);
    }
...
/*--------------------------------------------------------------------------*/
static gchar *
get_fullpath_from_fileinfo(DoryFileInfo *fileinfo)
{
  GFile *file;
  gchar *fullpath;
  ...
  file = dory_file_info_get_location(fileinfo);
...
static PropertyPage *
create_property_page (DoryFileInfo *fileinfo)
{
  PropertyPage *page;
  GError *error;
  ShareInfo *share_info;
  ...
  gtk_builder_set_translation_domain (page->xml, "dory-extensions");
...
/* Implementation of the DoryInfoProvider interface */

/* dory_info_provider_update_file_info
 * This function is called by Dory when it wants the extension to
 * fill in data about the file.  It passes a DoryFileInfo object,
 ...
 * the extension can do so, and return DORY_OPERATION_COMPLETE.
 ...
 * DORY_OPERATION_IN_PROGRESS.  The function must also set the
...
static DoryShareStatus
get_share_status_and_free_share_info (ShareInfo *share_info)
{
  DoryShareStatus result;

  if (!share_info)
    result = DORY_SHARE_NOT_SHARED;
  else
    {
      if (share_info->is_writable)
	result = DORY_SHARE_SHARED_RW;
      else
	result = DORY_SHARE_SHARED_RO;
...
/*--------------------------------------------------------------------------*/
static void
get_share_info_for_file_info (DoryFileInfo *file, ShareInfo **share_info, gboolean *is_shareable)
{
  char		*uri;
  ...
  *is_shareable = FALSE;
  ...
  f = dory_file_info_get_location(file);
  if (!uri)
    goto out;
...
#define NETWORK_SHARE_PREFIX "network:///share-"
  ...
  if (!dory_file_info_is_directory(file))
    goto out;
...
/*--------------------------------------------------------------------------*/
static DoryShareStatus
file_get_share_status_file(DoryFileInfo *file)
{
  ShareInfo *share_info;
  ...
  if (!is_shareable)
    return DORY_SHARE_NOT_SHARED;

  return get_share_status_and_free_share_info (share_info);
}

static DoryOperationResult
dory_share_update_file_info (DoryInfoProvider *provider,
				 DoryFileInfo *file,
				 GClosure *update_complete,
...
/*   gchar *share_status = NULL; */
  ...
  case DORY_SHARE_SHARED_RO:
    dory_file_info_add_emblem (file, "shared");
/*     share_status = _("shared (read only)"); */
    break;

  case DORY_SHARE_SHARED_RW:
    dory_file_info_add_emblem (file, "shared");
/*     share_status = _("shared (read and write)"); */
    break;

  case DORY_SHARE_NOT_SHARED:
...
/*   dory_file_info_add_string_attribute (file, */
/* 					   "DoryShare::share_status", */
/* 					   share_status); */
...
static void
dory_share_cancel_update (DoryInfoProvider *provider,
			      DoryOperationHandle *handle)
{
  DoryShareHandle *share_handle;

  share_handle = (DoryShareHandle*)handle;
  share_handle->cancelled = TRUE;
}

static GList *
dory_share_get_name_and_desc (DoryNameAndDescProvider *provider)
{
    GList *ret = NULL;
    gchar *string = g_strdup_printf ("dory-share:::%s",
...
static void
dory_share_info_provider_iface_init (DoryInfoProviderIface *iface)
{
  iface->update_file_info = dory_share_update_file_info;
  iface->cancel_update = dory_share_cancel_update;
}

static void
dory_share_nd_provider_iface_init (DoryNameAndDescProviderIface *iface)
{
    iface->get_name_and_desc = dory_share_get_name_and_desc;
}

/*--------------------------------------------------------------------------*/
/* dory_property_page_provider_get_pages
 *
 * This function is called by Dory when it wants property page
 * items from the extension.
 *
 * This function is called in the main thread before a property page
 * is shown, so it should return quickly.
 *
 * The function should return a GList of allocated DoryPropertyPage
 * items.
 */
static GList *
dory_share_get_property_pages (DoryPropertyPageProvider *provider,
				   GList *files)
{
  PropertyPage *page;
  GList *pages;
  DoryPropertyPage *np_page;
  ...
  fileinfo = DORY_FILE_INFO (files->data);
  ...
  pages = NULL;
  np_page = dory_property_page_new
    ("DoryShare::property_page",
...
/*--------------------------------------------------------------------------*/
static void
dory_share_property_page_provider_iface_init (DoryPropertyPageProviderIface *iface)
{
  iface->get_pages = dory_share_get_property_pages;
}

/*--------------------------------------------------------------------------*/
static void
dory_share_instance_init (DoryShare *share)
{
}

/*--------------------------------------------------------------------------*/
static void
dory_share_class_init (DoryShareClass *class)
{
 ...
 *
 * This function is called by Dory when it wants context menu
 * items from the extension.
 *
 * This function is called in the main thread before a context menu
 * is shown, so it should return quickly.
 *
 * The function should return a GList of allocated DoryMenuItem
 * items.
 */
...
static void
share_this_folder_callback (DoryMenuItem *item,
			    gpointer user_data)
{
  DoryFileInfo *fileinfo;
  PropertyPage *page;
  ...
  fileinfo = DORY_FILE_INFO (user_data);
  g_assert (fileinfo != NULL);
...
static GList *
dory_share_get_file_items (DoryMenuProvider *provider,
			     GtkWidget *window,
			     GList *files)
{
  GList *items;
  ...
  item = dory_menu_item_new ("DoryShare::share",
				 _("Sharing Options"),
				 _("Share this Folder"),
				 "folder-remote");
...
/*--------------------------------------------------------------------------*/
static void
dory_share_menu_provider_iface_init (DoryMenuProviderIface *iface)
{
	iface->get_file_items = dory_share_get_file_items;
}
...
#define DORY_TYPE_SHARE  (dory_share_get_type ())

static GType
dory_share_get_type (void)
{
  return share_type;
}

static void
dory_share_register_type (GTypeModule *module)
{
  static const GTypeInfo info = {
    sizeof (DoryShareClass),
    (GBaseInitFunc) NULL,
    (GBaseFinalizeFunc) NULL,
    (GClassInitFunc) dory_share_class_init,
    NULL,
    NULL,
    sizeof (DoryShare),
    0,
    (GInstanceInitFunc) dory_share_instance_init,
  };
  ...
  static const GInterfaceInfo property_page_provider_iface_info = {
    (GInterfaceInitFunc) dory_share_property_page_provider_iface_init,
    NULL,
    NULL
  };
  ...
  static const GInterfaceInfo info_provider_iface_info = {
    (GInterfaceInitFunc) dory_share_info_provider_iface_init,
    NULL,
    NULL
  };
  ...
  static const GInterfaceInfo menu_provider_iface_info = {
    (GInterfaceInitFunc) dory_share_menu_provider_iface_init,
    NULL,
    NULL
  };
  ...
  static const GInterfaceInfo nd_provider_iface_info = {
    (GInterfaceInitFunc) dory_share_nd_provider_iface_init,
    NULL,
    NULL
    };
 ...
 * dory-extensions-types.h, and must be implemented by all
 * extensions. */
...
{
  /*g_print ("Initializing dory-share extension\n");*/

  bindtextdomain("dory-extensions", DORY_SHARE_LOCALEDIR);
  bind_textdomain_codeset("dory-extensions", "UTF-8");

  dory_share_register_type (module);
}
...
{
  /*g_print ("Shutting down dory-share extension\n");*/
  /* FIXME freeing */
}
